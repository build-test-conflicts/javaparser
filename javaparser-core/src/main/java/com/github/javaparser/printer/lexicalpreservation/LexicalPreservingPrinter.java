 ...
 */
  public static DataKey<NodeText> NODE_TEXT_DATA=new DataKey<NodeText>(){
  }
;
  private static LexicalDifferenceCalculator LEXICAL_DIFFERENCE_CALCULATOR=new LexicalDifferenceCalculator();
  /** 
 * Prepares the node so it can be used in the print methods. The correct order is: <ol> <li>Parse some code</li> <li>Call this setup method on the result</li> <li>Make changes to the AST as desired</li> <li>Use one of the print methods on this class to print out the original source code with your changes added</li> </ol>
 ...
 */
  ...
  public static AstObserver createObserver(){
...
private static class Observer extends PropagatingAstObserver {
    ...
    public int getIndexOfComment(    Comment oldValue,    NodeText nodeText){
      int index;
    ...
    public List<ChildTextElement> findChildTextElementForComment(    Comment oldValue,    NodeText nodeText){
    ...
    public List<TokenTextElement> findTokenTextElementForComment(    Comment oldValue,    NodeText nodeText){
    ...
    public boolean isEqualRange(    Optional<Range> range1,    Optional<Range> range2){
    ...
    /** 
 * This method inserts new space tokens at the given  {@code index}. If a new comment is added to the node at the position of  {@code index}, the new comment and the node will have the same indent.
 * @param nodeText The text of the node
 * @param index The position where a new comment will be added to
 */
    public void fixIndentOfMovedNode(    NodeText nodeText,    int index){
      if (index <= 0) {
        return;
      }
    ...
    @Override public void concreteListChange(    NodeList changedList,    AstObserver.ListChangeType type,    int index,    Node nodeAddedOrRemoved){
    ...
    @Override public void concreteListReplacement(    NodeList changedList,    int index,    Node oldValue,    Node newValue){
      NodeText nodeText=getOrCreateNodeText(changedList.getParentNodeForChildren());
  ...
  public static void storeInitialText(  Node root){
...
.visitBreadthFirst(root);
  ...
  public static Node findNodeForToken(  Node node,  Range tokenRange){
    if (PhantomNodeLogic.isPhantomNode(node)) {
      return null;
    }
  ...
  public static Iterator<TokenTextElement> tokensPreceeding(  final Node node){
 ...
 */
  ...
  public static void prettyPrintingTextNode(  Node node,  NodeText nodeText){
 ...
 */
public static NodeText interpret(Node node,CsmElement csm,NodeText nodeText){
...
if (child.getComment().isPresent()) nodeText.addToken(getTokenKind(child.getComment().get()),child.getComment().get().toString());
...
public static int getTokenKind(Comment comment){
if (comment instanceof LineComment) return Kind.SINGLE_LINE_COMMENT.getKind();
if (comment instanceof BlockComment) return Kind.MULTI_LINE_COMMENT.getKind();
...
public static boolean isReturningOptionalNodeList(Method m){
