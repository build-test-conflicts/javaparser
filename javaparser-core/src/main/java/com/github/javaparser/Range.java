...
public class Range {
  ...
  public Position end;
  /** 
 * A range of characters in a source file, from "begin" to "end". This range is inclusive of the characters at the "begin" and "end" positions. <p> Note that if the given parameters are reversed (i.e. the end is earlier than begin, then the values are swapped.
 ...
 */
  ...
  /** 
 * Create a new `Range` object using the given begin and end line/column values. Valid values for each parameter are per the constructor of {@link Position}.
 ...
 */
  ...
  /** 
 * @param beginColumn The value used to replace the current begin column number.Valid values are per the constructor of {@link Position}.
 ...
 */
  ...
  /** 
 * @param beginLine The value used to replace the current begin line number.Valid values are per the constructor of {@link Position}.
 ...
 */
  ...
  /** 
 * @param endColumn The value used to replace the current end column number.Valid values are per the constructor of {@link Position}.
 ...
 */
  ...
  /** 
 * @param endLine The value used to replace the current end line number.Valid values are per the constructor of {@link Position}.
 ...
 */
  ...
  /** 
 * Does this loosely contain the other range? <p> As  {@link #strictlyContains(Range)}, but also allow ranges which have an equal start and/or end position. In these cases, the `other` range is not strictly "inside" of this range.
 */
  public boolean contains(  Range other){
    return (begin.isBefore(other.begin) || begin.equals(other.begin)) && (end.isAfter(other.end) || end.equals(other.end));
  }
  /** 
 * Does this loosely contain the other range? <p> As  {@link #strictlyContains(Position)}, but a position that is on the "edge" of this range will also pass. <p> For example, if the given position is equal to the start or end position of this range. In these cases, the `other` range is not strictly "inside" of this range.
 */
  public boolean contains(  Position position){
    return strictlyContains(position) || begin.equals(position) || end.equals(position);
  }
  /** 
 * Does this strictly contain the other range? <p> It means that this has to be larger than other and it has to start before other and end after other.
 */
  public boolean strictlyContains(  Range other){
    return begin.isBefore(other.begin) && end.isAfter(other.end);
  }
  /** 
 * Does this strictly contain position. <p> It means that the position is after the begin of this range and before the end of this range.
 */
  public boolean strictlyContains(  Position position){
    return position.isAfter(begin) && position.isBefore(end);
  }
  /** 
 * Does the other 'Range' overlap with this 'Range'? <p> If two ranges overlap, this range or the other range contains the begin or the end of the other range. <p> Note that if the ends are "touching" (i.e. a begin position == end position), this counts as an overlap because the positions refer to characters, as opposed to boundary between characters. <p> For example, there is an overlap at "C" in the following ranges, with "C" existing within both ranges: <pre> Range 1: ABC Range 2:   CDE</pre>
 */
 ...
 */
  ...
  /** 
 * @return The number of lines that this range represents.<p> If the start line and end line are the same, this range is limited to just one line.
 */
