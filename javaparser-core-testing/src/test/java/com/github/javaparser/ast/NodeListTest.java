...
class NodeListTest {
  public AstObserver createObserver(  List<String> changes){
    return new AstObserver(){
      ...
      @Override public void parentChange(      Node observedNode,      Node previousParent,      Node newParent){
      ...
      @Override public void listReplacement(      NodeList observedNode,      int index,      Node oldNode,      Node newNode){
...
;
  ...
  public FieldDeclaration createIntField(  String name){
    return new FieldDeclaration(new NodeList<>(),PrimitiveType.intType(),name);
  }
  @Test void addAllWithoutIndex(){
  ...
  @Test void removeAll(){
    ...
    assertEquals(Arrays.asList("'int b;' REMOVAL in list at 1","'int d;' REMOVAL in list at 2"),changes);
  }
  @Test void retainAll(){
    List<String> changes=new LinkedList<>();
    String code="class A { int a; int b; int c; int d; int e; }";
    ...
    assertEquals(Arrays.asList("'int a;' REMOVAL in list at 0","'int c;' REMOVAL in list at 1","'int e;' REMOVAL in list at 2"),changes);
  }
  @Test void replaceAll(){
    List<String> changes=new LinkedList<>();
    String code="class A { int a; int b; int c; }";
    CompilationUnit cu=parse(code);
    ClassOrInterfaceDeclaration cd=cu.getClassByName("A").get();
